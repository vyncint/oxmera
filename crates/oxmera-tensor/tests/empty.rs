...
fn sum_of_nothing_is_zero_for_every_empty_shape() {
    for dims in [&[0usize][..], &[0, 3], &[2, 0], &[2, 0, 5], &[0, 0]] {
        ...
        assert_eq!(t.sum(&all).unwrap().to_vec_f32().unwrap(), vec![0.0], "{dims:?}");
    }
}
...
fn max_and_min_of_nothing_are_their_identities() {
    let t = empty(&[0, 3]);
    assert_eq!(t.max(&[0, 1]).unwrap().to_vec_f32().unwrap(), vec![f32::NEG_INFINITY]);
    assert_eq!(t.min(&[0, 1]).unwrap().to_vec_f32().unwrap(), vec![f32::INFINITY]);
}
...
fn reducing_only_the_empty_axis_keeps_the_others() {
    ...
    assert!(m.to_vec_f32().unwrap().iter().all(|&x| x == f32::NEG_INFINITY));
}
...
fn mean_of_nothing_is_nan_not_a_panic() {
    ...
    assert_eq!(m.len(), 1);
    assert!(m[0].is_nan(), "0/0 → NaN, as in IEEE and NumPy; got {}", m[0]);
}
